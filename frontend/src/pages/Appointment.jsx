    ...
    docInfo && (
      <div>
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
          ...
          <div className="flex gap-3 items-center w-full mt-4 overflow-x-scroll">
            {docSlots.length &&
              docSlots.map((item, index) => (
                <div
                  className={`text-center rounded-full cursor-pointer py-6 min-w-16 ${
                    slotIndex === index
          ...
          <div className="flex items-center gap-3 w-full mt-4 overflow-x-scroll">
            {docSlots.length &&
              docSlots[slotIndex].map((item, index) => (
                <p
                  className={`text-sm font-light cursor-pointer rounded-full px-5 py-2 flex-shrink-0 ${
                    item.time === slotTime
                      ? "bg-primary text-white"
          ...
          <button className="rounded-full text-white tex-sm bg-primary font-light px-14 py-3 my-6">
            Book an Appointment
          </button>
        </div>
