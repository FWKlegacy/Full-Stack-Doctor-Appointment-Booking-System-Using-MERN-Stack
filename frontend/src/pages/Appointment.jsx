				...
				<div className='flex flex-col sm:flex-row gap-4'>
					...
					<div className='flex-1 border border-gray-400 p-8 py-7 mx-2 bg-white sm:mx-0 mt-[-80px] rounded-lg sm:mt-0'>
						...
						<div>
							...
							<p className='text-sm mt-1 max-w-[700px] text-gray-500'>{docInfo.about}</p>
						</div>
					...
					<div className='flex gap-3 items-center w-full mt-4 overflow-x-scroll'>
						{docSlots.length &&
							docSlots.map((item, index) => (
								<div className={`text-center rounded-full cursor-pointer py-6 min-w-16 ${slotIndex === index ? 'bg-primary text-white' : 'border border-gray-200'}`} onClick={() => setSlotsIndex(index)} key={index}>
					...
					<div className='flex items-center gap-3 w-full mt-4 overflow-x-scroll'>
						{docSlots.length &&
							docSlots[slotIndex].map((item, index) => (
								<p className={`text-sm font-light cursor-pointer rounded-full px-5 py-2 flex-shrink-0 ${item.time === slotTime ? 'bg-primary text-white' : 'text-gray-400 border border-gray-300'}`} onClick={() => setSlotsTime(item.time)} key={index}>
									{item.time.toLowerCase()}
								</p>
							))}
					</div>
					<button className='rounded-full text-white tex-sm bg-primary font-light px-14 py-3 my-6'>Book an Appointment</button>
				</div>
